...
3. 在 CLI 输入 `start` 开始自动搜索、滚动加载职位、分析职位和打招呼。
...
- 如果 CLI 没有出现 `脚本就绪: 2026.06-cli-autogreet.17`，说明油猴脚本没有更新或没有连接到本地 API；优先运行 `script` 并通过本地安装地址更新。
- 如果出现 `未匹配页面路径: /xian/` 这类城市首页日志，请更新脚本到 `2026.06-cli-autogreet.17`；新版会自动进入标准职位搜索页。
- 如果出现 `接收超时: get-job-info`，请更新脚本到 `2026.06-cli-autogreet.17`；新版把详情页来源有效期放宽到 120 秒，并且脚本打开的详情窗口会按窗口名回传职位信息。
- 如果 CLI 报 `验证码` 但页面肉眼没有验证码，请更新到 `2026.06-cli-autogreet.17`；新版不再因为普通“验证码”文案误判，只在明确的验证/风控页面暂停。
- 本次打招呼上限按一轮自动化运行计数，`pause/start` 继续同一轮不清零；`stop` 后下一次 `start` 会开启新一轮并清零。达到上限后脚本会通知后端进入 `stopped`，不再继续打开岗位或调用模型。
- 搜索页会优先滚动加载更多岗位；连续 3 次滚动后仍没有新岗位时，脚本会自动切换到下一个岗位标签重新搜索。
...
- 如果 CLI 多次显示 `发现职位链接 N 个，新职位 0 个`，说明当前搜索结果已经没有未处理的新职位；脚本会先滚动重试，仍无新职位才切换到下一个岗位标签。
