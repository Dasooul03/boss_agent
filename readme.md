...
3. 在 CLI 输入 `start`，先确认本轮岗位标签和本次打招呼上限，再开始自动搜索、分析职位和打招呼。
...
- 如果 CLI 没有出现 `脚本就绪: 2026.06-cli-autogreet.18`，说明油猴脚本没有更新或没有连接到本地 API；优先运行 `script` 并通过本地安装地址更新。
- 如果出现 `未匹配页面路径: /xian/` 这类城市首页日志，请更新脚本到 `2026.06-cli-autogreet.18`；新版会自动进入标准职位搜索页。
- 如果出现 `接收超时: get-job-info`，请更新脚本到 `2026.06-cli-autogreet.18`；新版把详情页来源有效期放宽到 120 秒，并且脚本打开的详情窗口会按窗口名回传职位信息。
- 如果 CLI 报 `验证码` 但页面肉眼没有验证码，请更新到 `2026.06-cli-autogreet.18`；新版不再因为普通“验证码”文案误判，只在明确的验证/风控页面暂停。
